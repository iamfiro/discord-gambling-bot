...
import { betHalfMoney, getUserMoney } from "../../db/money";
...
async function handler(interaction: ChatInputCommandInteraction) {
    checkAvailableUser(interaction).then(async function(verficationResult) {
        ...
        getUserMoney(interaction).then(async function(moneyResult) {
            const halfMoney:bigint = BigInt(Math.floor(Number(moneyResult) / 2))
            if(!onlyNumberRegex.test(halfMoney.toString())) {
            ...
            betHalfMoney(interaction, randomScale, Number(halfMoney)).then(async function(result) {
                ...
                .addFields({ name: '\u200B', value: '\u200B' },{ 
                    ...
                    name: '베팅 금액 🧾', 
                    value: numberWithCommas(Number(halfMoney)).toString() + '원\n', 
                    inline: true,
                }, { 
                    name: '베팅 수익 💸', 
                    value: numberWithCommas(BigInt(Math.floor(Number(halfMoney) * randomScale)) - halfMoney).toString() + '원', 
                    inline: true,
                }, { 
                    name: '내 잔고 💰', 
...
export default {
    info: new SlashCommandBuilder()
        .setName("하프")
        .setDescription("💸 현재 가지고 있는 잔액의 절반을 베팅합니다"),
    handler
}